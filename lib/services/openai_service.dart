...
class OpenAIService {
  ...
  List<Map<String, dynamic>> _chatMessagesToGeminiContents(
  ...
  Stream<String> getChatResponse({
    ...
    if (model.toLowerCase().startsWith('google/gemini') ||
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            if (data['candidates'] != null &&
              ...
              throw Exception(
                'No valid response candidates found in Gemini API response. Body: ${response.body}',
              );
            }
          } else {
            print(
              'API Error: ${response.statusCode} for model $model at $endpointUrl',
            );
            print('API Response: ${response.body}');
            String errorMessage =
                'Failed to get response from API: ${response.statusCode} ${response.reasonPhrase}';
            try {
            ...
            if (response.headers['content-type']?.toLowerCase().contains(
                ...
                false) {
              throw Exception(
                'Failed to connect to API: Received HTML response (Status ${response.statusCode}). Please check Provider URL, API key, and ensure the Generative Language API is enabled in your Google Cloud project.',
              );
            }
      ...
      Stream<String> generateOpenAIResponse() async* {
        try {
          ...
          if (streamedResponse.statusCode == 200) {
            ...
            print(
              'API Error: ${streamedResponse.statusCode} for model $model at $endpointUrl',
            );
            print('API Response: $responseBody');
            String errorMessage =
                'Failed to get response from API: ${streamedResponse.statusCode} ${streamedResponse.reasonPhrase}';
            try {
